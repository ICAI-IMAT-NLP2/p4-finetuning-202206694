...
def inject_lora_into_model(model, r=4, alpha=32, device='cpu'):
    ...
    for child_name, child_module in model.named_children():
        ...
        if child_module.__class__.__name__ == "T5Attention":
            # TODO: Create LoRA layer for linear module
            for proj_name in ("q", "k", "v", "o"):
                if hasattr(child_module, proj_name):
                    lin = getattr(child_module, proj_name)
                    if isinstance(lin, nn.Linear):
                        lora_layer = LoRA(lin, r=r, alpha=alpha)
                        setattr(child_module, proj_name, lora_layer)
        else:
